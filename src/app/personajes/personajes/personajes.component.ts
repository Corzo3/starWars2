...
export class PersonajesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.personajeService
      ...
      .subscribe(
        (response) =>
          (this.personajes = this.personajeService.extraerPersonajes(response))
      );
  ...
  getTodosPersonajes(): void {
    ...
    this.personajeService.getPersonajes().subscribe((r) => {
      ...
      for (let index = 1; index <= this.numPaginas; index++) {
        this.personajeService
          .getPersonajesPagina(index)
          .subscribe((response) => {
            this.todosPersonajes.push(
              ...this.personajeService.extraerPersonajes(response)
            );
          });
      }
    });
